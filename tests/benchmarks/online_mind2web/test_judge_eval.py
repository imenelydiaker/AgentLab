...
class TestJudgeEval:
    ...
    @patch('agentlab.benchmarks.online_mind2web.judge_eval.encode_image', return_value="base64encodedstring")
    @patch('agentlab.benchmarks.online_mind2web.judge_eval.identify_key_points', return_value="1. Point one.\n2. Point two.")
    @patch('agentlab.benchmarks.online_mind2web.judge_eval.judge_image', return_value="**Score**: 4\n\n**Reasoning**: The screenshot shows the correct action." )
    def test_webjudge_online_mind2web_eval_with_image(self, mock_encode_image, mock_identify_key_points, mock_judge_image):
        import numpy as np
        from PIL import Image
        ...
        task_instruction = "Test task instruction."
        screenshots = [Image.fromarray(np.zeros((100, 100, 3), dtype=np.uint8), "RGB") for _ in range(3)]
    ...
    @pytest.mark.xfail(reason="This test currently fails, an image is still being passed to the judge.")
    @patch('agentlab.benchmarks.online_mind2web.judge_eval.encode_image', return_value="base64encodedstring")
    @patch('agentlab.benchmarks.online_mind2web.judge_eval.identify_key_points', return_value="1. Point one.\n2. Point two.")
    @patch('agentlab.benchmarks.online_mind2web.judge_eval.judge_image', return_value="Score: 4\n\nReasoning: The screenshot shows the correct action." )
    def test_webjudge_online_mind2web_eval_no_image(self, mock_encode_image, mock_identify_key_points, mock_judge_image):
        import numpy as np
        from PIL import Image
